    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center">
                ...
                <h3> a place that prompts you to think critically and specifically about the news you read.</h3>
                <form onSubmit={handleSubmit} className="flex mt-8">
                    ...
                    />
                    <button type="submit" className="btn btn-circle btn-outline">GO</button>
                </form>
                {questions.length > 0 && (
                    <div className="mt-8 w-full max-w-md space-y-4">
                        {questions.map((q, index) => (
                            <div
                                ...
                                onClick={() => {setQuestionKeywords(q.keywords); setQuestion(q.question); navigate(`/${user.id}/pulsecheck-articles`)}}
                            >
                                {q.question}
                            </div>
                        ))}
