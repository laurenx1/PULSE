...
router.post('/generate-pulsecheck-response', async (req, res) => {
  ...
  const prompt =
    'Four questions one could ask Google about (and an array of 5 single-word keywords for each question) relating to the following topic and current events: ';
      ...
      messages: [
        {
          role: 'system',
          content:
            'do not provide any text but the questions and the array (no preceding response). Please provide them in the format Question 1: Question Keywords: []',
        },
        {
          role: 'user',
