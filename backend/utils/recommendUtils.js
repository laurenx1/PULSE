...
const calculateUserSimilarity = (user1, user2) => {
  ...
  const similarityScore =
    weight1 * commonLikes.length +
    weight2 * commonSaves.length +
    weight3 * commonTopics.length +
    weight4 * commonLikeToSave.length +
    weight5 * commonSaveToLike.length;
...
const recommendArticles = (targetUser, similarUsers, allArticles) => {
  ...
  similarUsers.forEach(similarUser => {
    ...
    similarUser.interactions.forEach(interaction => {
      if (
        (interaction.liked || interaction.saved) &&
  ...
  const sortedArticleIds = Object.keys(articleScores).sort(
    (a, b) => articleScores[b] - articleScores[a],
  );

  // get articleIds of the articles so they can be fetched to target user
  return sortedArticleIds.map(articleId =>
    allArticles.find(article => article.id === parseInt(articleId)),
  );
};
